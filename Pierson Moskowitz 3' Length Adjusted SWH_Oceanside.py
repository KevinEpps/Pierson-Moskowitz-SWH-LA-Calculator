...
for i in range(len(time_array)):
    ...
    sea_modal_period = 1 / max_sea_energy_density_freq
    ...
    if sea_modal_period < 7:
      length_adjusted_height * 1.05
    else:
      length_adjusted_height * 1

    if (swell_direction - sea_direction) < 40:
      length_adjusted_height * 1.025
    else:
      length_adjusted_height * 1

    results.append([time_array[i], current_waveHs, cm, measured_sig_height, length_adjusted_height, sea_modal_period, sea_direction])

# Create a dataframe using the calculated values for each time step
results_df = pd.DataFrame(results, columns=['Time', 'Buoy Wave Height (ft)', 'CM Normalization Factor', 'Calculated Total Height', 'Length Adjusted Height', 'Sea Modal Period', 'Sea Direction'])
...
print('Calculated Total Height', round(measured_sig_height, 2))

# Prints the 'Length Adjusted Height', which is the 3' normalized PM SWH
print('Length Adjusted Height', round(length_adjusted_height, 2))
...
'''
# Sea Modal Period over time
plt.figure(figsize=(12, 6))
plt.plot(results_df['Time'], results_df['Sea Modal Period'], label='Sea Modal Period')
plt.legend(loc='upper left')
plt.title('Sea Modal Period over Time')
plt.xlabel('Time')
plt.ylabel('Modal Period (s)')
plt.xticks(rotation=45)
plt.tight_layout()
plt.show()
'''
'''
# Sea Direction over time
plt.figure(figsize=(12, 6))
plt.plot(results_df['Time'], results_df['Sea Direction'], label='Sea Direction')
plt.legend(loc='upper left')
plt.title('Sea Direction over Time')
plt.xlabel('Time')
